...
# 
# 安装Hadoop 2集群(需用到pdsh和pdcp)
# 
# 脚本可以以交互模式运行,也可以基于文件自动运行.
# 脚本以基本配置来创建一个Hadoop 2集群.可以根据你的需求
# 来更改其中的data,log,pid目录等环境配置.
# 在本脚本运行之后,如果想更进一步的配置集群,可以使用另一个
# 脚本configure-hadoop2.sh
# 
...
install()
{
    ...
    pdsh -w ^all_hosts tar -xzf /opt/hadoop-"$HADOOP_VERSION".tar.gz -C /opt
    ...
    hadoop jar $HADOOP_HOME/share/hadoop/mapreduce/hadoop-mapreduce-examples-$HADOOP_VERSION.jar pi \
    -Dmapreduce.clientfactory.class.name=org.apache.hadoop.mapred.YarnClientFactory \
    -libjars $HADOOP_HOME/share/hadoop/mapreduce/hadoop-mapreduce-client-jobclient-$HADOOP_VERSION.jar 16 10000
}
...
OPTIONS:
  -i,  --interactive    Prompt for fully qualified domain names(FQDN) of the NameNode,
                        Secondary NameNode, DataNodes, ResourceManager, NodeManager,
                        MapReduce Job History Server, and YARN Proxy Server, Values 
                        entered are stored in files in the same directory as this 
                        command.
  -f,  --file           Use files with fully qualified domain names(FQDN), newline as 
